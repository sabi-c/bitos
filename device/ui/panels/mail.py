...
class MailPanel(BasePanel):
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        title_surf = font_body.render("MAIL", False, BLACK)
        ...
        for idx, email in enumerate(self.emails):
            ...
            sender_surf = font_body.render(email["sender"][:16], False, from_color)
            time_surf = font_cap.render(email["time"], False, GRAY_333 if not focused else BLACK)
            surface.blit(sender_surf, (8, y + 3))
            surface.blit(time_surf, (PANEL_W - time_surf.get_width() - 8, y + 4))

            # Subject line
            subj_surf = font_cap.render(email["subject"][:20], False, subj_color)
        ...
        y += 2
        footer_surf = font_cap.render("\u25b6 OPEN FULL INBOX", False, GRAY_222)
