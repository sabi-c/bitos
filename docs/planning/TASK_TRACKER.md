...
|---|---|---|---|---|---|---|---|
| CODEX-UI-001 | Phase 11 | Remote Control | Codex App Server migration architecture lock + phased plan | GPT-5.2-Codex | done | 2026-03-20 | Added `docs/planning/CODEX_REMOTE_CONTROL_MIGRATION.md` and aligned planning surfaces to unified BFF/App Server direction |
| CODEX-UI-002 | Phase 11 | Remote Control | Build vertical slice endpoints for session/turn/event/approval flows | GPT-5.2-Codex | done | 2026-03-20 | Added mock Codex app client + `/api/codex/*` endpoints + focused endpoint tests (`tests/test_codex_remote_endpoints.py`) |
...
| MVP-001 | Phase 1 | Emulator | Desktop emulator + full run loop for Mac/Pygame workflows | GPT-5.2-Codex | done | 2026-03-14 | Added Makefile run-dev/run-server/run-both, keyboard gesture mapping, and mock voice-input chat loop |
| MVP-002 | Phase 1 | Hardware Prep | Pi hardware driver stubs + first boot verification prep | GPT-5.2-Codex | done | 2026-03-14 | Added ST7789 hardware path, GPIO button handler, verify script, and Pi run Makefile targets |
| MVP-004 | Phase 1 | Hardware Prep | Final pre-hardware checklist + env/template + smoke validation workflow | GPT-5.2-Codex | done | 2026-03-14 | Updated .env.template/README, added smoke + tracker validator scripts, normalized tracker sections |
| AUDIT-FINAL | Phase 1 | Hardware Prep | Connection audit before first hardware boot + critical blocker fixes | GPT-5.2-Codex | done | 2026-03-14 | Added boot readiness report, fixed package import chain, and added missing install script |
...
| CI-001 | Phase 0 | Tooling | Add GitHub Actions CI workflow for tracker validation, tests, and companion JS file checks | GPT-5.2-Codex | done | 2026-03-14 | Added `.github/workflows/ci.yml` and wired README status badge. |
| P2-001 | Phase 2 | WS-A App Shell | Add lock screen + route from boot | GPT-5.2-Codex | done | 2026-03-14 | Implemented boot -> lock -> home flow in device runtime; added shell flow tests |
| P0-002 | Phase 0 | Planning | Publish explicit handoff brief for next contributor with immediate next slice | GPT-5.2-Codex | done | 2026-03-14 | Added `docs/planning/HANDOFF_NEXT_AGENT.md` and linked it from key docs |
| P0-AUDIT-002 | Phase 0 | Quality Audit | Run repository test coverage audit and publish prioritized coverage report | GPT-5.2-Codex | done | 2026-03-14 | Added `docs/reports/TEST_COVERAGE.md` with untested modules and priority targets |
...
| P5-012a | Phase 5 | WS-H Companion PWA | Build setup.html multi-step BLE provisioning UI | GPT-5.2-Codex | done | 2026-03-14 | Added standalone companion setup flow UI with connect/auth/wifi/done steps |
...
| SHIP-PREP-001 | Phase 5 | WS-F Infra & Security | Hardware port readiness pass (Pi audit, voice loop, Vikunja, boot polish) | Claude | done | 2026-03-14 | All Pi code paths verified complete, voice capture threaded with TTS playback, Vikunja adapter with mock fallback, boot status messages for first-boot/offline/no-key, main.py critical bugs fixed |
...
|---|---|---|---|---|---|---|
| 2026-03-20 | GPT-5.2-Codex | work | `this-commit` | Completed CODEX-UI-001 planning iteration for unified Codex remote-control architecture and backlog alignment | `docs/planning/CODEX_REMOTE_CONTROL_MIGRATION.md`, `docs/planning/HANDOFF_NEXT_AGENT.md`, `docs/planning/IMPLEMENTATION_PLAN_NEXT.md`, `docs/planning/TASK_TRACKER.md`, `README.md` | Next: implement CODEX-UI-002 vertical slice endpoints + tests |
| 2026-03-20 | GPT-5.2-Codex | work | `this-commit` | Completed CODEX-UI-002 vertical slice scaffold (mock Codex app client, remote endpoints, router wiring, endpoint tests) | `server/codex_app_client.py`, `server/endpoints/codex_remote.py`, `server/main.py`, `tests/test_codex_remote_endpoints.py`, `docs/planning/TASK_TRACKER.md` | Next: replace mock transport with real App Server stdio/websocket adapter behind same interface |
...
| 2026-03-14 | GPT-5.2-Codex | work | `this-commit` | Completed P5-012a/b/c/d companion PWA Wi-Fi provisioning flow (setup UI + BLE/auth/crypto JS + manifest/index/readme) | `companion/setup.html`, `companion/js/ble.js`, `companion/js/auth.js`, `companion/js/crypto.js`, `companion/manifest.json`, `companion/index.html`, `companion/README.md`, `README.md`, `docs/planning/TASK_TRACKER.md` | Next: P5-012e/f PWA install polish + cross-platform QA checklist |
